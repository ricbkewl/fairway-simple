...
function positionExpandedGuide(details){
  ...
  const createJoin=details.querySelector('.guide-body > h3:nth-of-type(2)')?.nextElementSibling;if(createJoin&&!createJoin.querySelector('[data-menu-guide]'))createJoin.insertAdjacentHTML('beforeend','<li data-menu-guide>Courses appears first in the main navigation. The gold Resume control includes a separate one-third End Round action with confirmation.</li>');
  const update=details.querySelector('.guide-update');if(update)update.textContent='Current feature guide · Version 94 · Updated August 31, 2026';
...
function bottomNav(){if(s.v==='round'){roundBottomNav();return}app.insertAdjacentHTML('beforeend',`<nav class="bottom-nav ${s.sharedRoundId?'has-chat':''}" aria-label="Main navigation"><button onclick="openCoursesFromNav()"><span>⛳</span>${adminRole==='super_admin'?'Courses/Players':'Courses'}</button><button onclick="goHome()"><span>⌂</span>Home</button>${s.sharedRoundId?'<button onclick="openCurrentRound()"><span>🏌</span>Round</button>':''}${s.sharedRoundId?`<button onclick="openRoundChat()"><span class="chat-nav-icon">💬<i id="chatUnreadBadge" class="chat-unread ${unreadChatCount?'':'hidden'}">${unreadChatCount>99?'99+':unreadChatCount}</i></span>Chat</button>`:''}<button onclick="accountAction()"><span>${currentUser?'●':'♙'}</span>${currentUser?'My Account':'Login'}</button></nav>`)}
