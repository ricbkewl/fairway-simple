...
function round(){
  ...
  ensureCurrentHolePar();const h=s.hole,p=s.pars[h-1],pt=parTotal(h),c=courseById(s.courseId),green=c?.greens?.[h-1],holeYards=mappedHoleDistance(green);
  app.innerHTML=`<div class="row round-heading"><div><div class="brand small-brand">Agape Tumoutou Golfers</div><span class="small muted">${esc(s.course)}</span> <span id="syncStatus" class="sync-status ${navigator.onLine?'live':'waiting'}">${navigator.onLine?'Live':'Offline'}</span></div><div class="round-heading-actions"><button class="scorecard-button" onclick="openScorecard()"><span>▦</span> Live Scorecard <i>${s.players.length}</i></button>${s.sharedRoundId?'<button class="round-menu-button" onclick="openRoundManagement()" aria-label="Round options">•••</button>':''}</div></div><section class="hole hole-focus map-first-heading"><div class="hole-count"><span>HOLE</span><strong>${h}</strong><small>of ${s.holes}</small><i>PAR ${p}</i></div><h1>${holeYards?`${holeYards} Yards · Tee to Center`:'Hole distance not mapped'}</h1></section>${green?liveHoleMapPanel(green,h,p)+yardagePanel():`<div class="notice">No GPS markers for this hole. Add them from Map & Manage Courses.</div>`}${s.players.filter(x=>isMyPlayer(x)).map(x=>{const mine=isMyPlayer(x);return`<div class="card score ${mine?'my-score':''}"><div><b>${esc(x)}${mine?' (You)':''}</b><div class="current-hole-reference">Scoring Hole ${h} · Par ${p}</div><div class="small muted">Total ${total(x,h)} · ${rel(total(x,h)-pt)}</div></div>${mine?`<div class="stepper"><button onclick="changeScore('${encodeURIComponent(x)}',-1)">−</button><span>${scoreValue(x)||'–'}</span><button onclick="changeScore('${encodeURIComponent(x)}',1)">+</button></div>`:`<span class="locked-score">${scoreValue(x)||'–'} 🔒</span>`}<span class="green">${scoreValue(x)?rel(scoreValue(x)-p):''}</span></div>`}).join('')}<div class="row"><button class="secondary half" onclick="prev()">Previous</button><button class="primary fit" onclick="next()">${h===s.holes?'Finish':'Next Hole →'}</button></div>`;
...
function initInlineHoleMap(green){
  ...
  inlineHoleMap=L.map(container,{zoomControl:false,dragging:false,scrollWheelZoom:false,doubleClickZoom:false,boxZoom:false,keyboard:false,touchZoom:false,attributionControl:false});
  ...
  const points=[green.tee,green.front,green.center,green.back].filter(Boolean);inlineHoleMap.fitBounds(points.map(point=>[point.lat,point.lng]),{padding:[72,48],maxZoom:18});
