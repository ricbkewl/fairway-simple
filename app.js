...
const LISTED_COURSE_CATALOG=[
  ...
  {id:'catalog-shorecliffs',name:'Shorecliffs Golf Club',city:'San Clemente',state:'CA',postal_code:'92672',address:'501 Avenida Vaquero',holes:18,par_total:71,course_type:'Public',catalog_point:{lat:33.427,lng:-117.612},source:'https://www.shorecliffsgolfclub.com/'},
  {id:'catalog-newport-beach',name:'Newport Beach Golf Course',city:'Newport Beach',state:'CA',postal_code:'92660',address:'3100 Irvine Avenue',holes:18,par_total:59,course_type:'Public · Executive',catalog_point:{lat:33.6189,lng:-117.9298},source:'https://www.newportbeachgolfcoursellc.com/'},
  {id:'catalog-willowick',name:'Willowick Golf Course',city:'Santa Ana',state:'CA',postal_code:'92703',address:'3017 West 5th Street',holes:18,par_total:71,course_type:'Public',catalog_point:{lat:33.7455,lng:-117.8677},source:'https://www.willowickgolf.com/'},
  ...
  {id:'catalog-jurupa-hills',name:'Jurupa Hills Country Club',city:'Riverside',state:'CA',postal_code:'92509',address:'6161 Moraga Avenue',holes:18,par_total:70,course_type:'Public',catalog_point:{lat:33.9806,lng:-117.3755},source:'https://www.jurupahills.net/'},
...
function mergeListedCourseCatalog(cloudCourses){
  ...
  for(const catalog of LISTED_COURSE_CATALOG){
    ...
    else merged.push({...catalog,pars:catalog.pars?[...catalog.pars]:[],greens:[],catalogOnly:true,catalogApproved:true});
  }
...
function positionExpandedGuide(details){
  ...
  const update=details.querySelector('.guide-update');if(update)update.textContent='Current feature guide · Version 95 · Updated August 31, 2026';
...
function setup(){const selectedCourseId=s.courseId||s.catalogCourseId,hasSavedCourse=!!selectedCourseId;const options=courses.map(c=>`<option value="${esc(c.id)}" ${selectedCourseId===c.id?'selected':''}>${esc(c.name)} (${c.holes} holes)</option>`).join('');app.innerHTML=`<button class="back" onclick="goHome()">← Back</button><h1>Create a Round</h1><p class="muted">Choose a saved course to start immediately, or create a custom scorecard.</p><label>Saved course</label><select id="savedCourse" onchange="chooseCourse(this.value)"><option value="">Custom scorecard without GPS</option>${options}</select>${hasSavedCourse?`<div class="notice">${s.courseId?'The saved GPS markers and hole pars will be used automatically.':'The approved course name and hole pars will be used. GPS guidance will appear after its hole mapping is verified.'}</div>`:`<label>Course name</label><input id="course" value="${esc(s.course)}" placeholder="e.g., Oak Valley Golf Club"><label>How many holes?</label><div class="row"><button class="choice ${s.holes===9?'on':''}" onclick="setHoles(9)">9 Holes</button><button class="choice ${s.holes===18?'on':''}" onclick="setHoles(18)">18 Holes</button></div>`}<label>Your name for this round</label><input aria-label="Your name for this round" value="${esc(s.players[0]||'')}" placeholder="Enter your name" oninput="updatePlayer(0,this.value)"><div class="notice">Your profile first name is entered automatically, but you can edit it. Other golfers join from their own phones.</div><button id="${hasSavedCourse?'createRoundButton':'setupContinueButton'}" class="primary" onclick="goPars()">${hasSavedCourse?'Create Protected Round':'Continue'}</button>`}
