...
function positionExpandedGuide(details){
  ...
  const update=details.querySelector('.guide-update');if(update)update.textContent='Current feature guide · Version 90 · Updated August 30, 2026';
...
function courseDistanceMiles(course){const point=coursePreviewPoint(course);return point&&courseLibraryLocation?distanceYards(courseLibraryLocation,point)/1760:null}
...
function refreshCourseLibrary(){
  ...
  const query=courseLibraryQuery,filtered=rankedSharedCourses().filter(item=>(!query||item.course.name.toLowerCase().includes(query))&&courseMatchesFilters(item.course));
...
function courseLibraryCard(course,index,distance=null){const point=coursePreviewPoint(course),favorite=favoriteCourseIds().has(course.id),difficulty=courseDifficulty(course);return`<article class="course-library-card"><div class="course-preview course-start-target" role="button" tabindex="0" onclick="startCourseFromLibrary(${index})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();startCourseFromLibrary(${index})}">${point?`<div id="coursePreview${index}" class="course-preview-map" data-lat="${point.lat}" data-lng="${point.lng}" aria-label="Satellite preview of ${esc(course.name)}"></div>`:''}<div class="course-preview-placeholder"><span>⛳</span><small>${point?'Loading satellite preview':'Map this course for a preview'}</small></div><button class="course-favorite ${favorite?'on':''}" onclick="toggleCourseFavorite('${esc(course.id)}',event)" aria-label="${favorite?'Remove':'Add'} ${esc(course.name)} ${favorite?'from':'to'} favorites" aria-pressed="${favorite}">${favorite?'★':'☆'}</button>${point?'<a href="https://www.maptiler.com/copyright/" target="_blank" rel="noopener" onclick="event.stopPropagation()">© MapTiler</a>':''}</div><div class="course-card-info"><div><small>${course.holes} HOLES · ${mappedCount(course)} MAPPED${distance===null?'':` · ${distance<10?distance.toFixed(1):Math.round(distance)} MI`}</small><button class="course-name-start" onclick="startCourseFromLibrary(${index})">${esc(course.name)}</button><span>${difficulty==='unknown'?'Difficulty pending':difficulty==='forward'?'Forward friendly':difficulty==='championship'?'Championship length':'Standard length'} · Tap to play</span></div>${adminRole?`<button onclick="editCourse(${index})">Edit</button>`:'<i>›</i>'}</div></article>`}
...
async function startCourseFromLibrary(index){const course=courses[index];if(!course)return;if(!confirm(`Would you like to start a new game at ${course.name}?`))return;rememberRecentCourse(course.id);const courseId=course.id;await start();if(s.v!=='setup')return;chooseCourse(courseId)}
...
async function approveIntelligentCourseMapping(){
  ...
  const reviewed=(draft.reviewedHoles||[]).filter(Boolean).length;if(reviewed<draft.holes){alert(`Review all ${draft.holes} holes before approving this mapping.`);return}
  if(!confirm('Approve and publish all Eagle Glen tee, aim and green markers?'))return;
  draft.greens.forEach(green=>green._review='approved');draft.intelligentDraft=false;await saveMappedCourse();
}
