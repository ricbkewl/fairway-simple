...
let shotPlannerEnabled=sessionStorage.atgShotPlanner==='on',inlinePlannerMarker=null,inlinePlannerLines=[],shotPlannerGreen=null;
...
function defaultShotPlannerAim(green){return activeRouteSegment(golferIsNearHole(green)?lastKnownPosition:null,green)?.target||green.center}
function shotPlannerAim(green){return shotPlannerAims[shotPlannerKey()]||defaultShotPlannerAim(green)}
...
function liveHoleMapPanel(green,h,p){
  ...
  return`<section class="live-hole-map ${shotPlannerEnabled?'planner-on':''}"><div class="live-map-viewport"><div id="liveHoleMap" aria-label="Forward-facing course view of Hole ${h}"></div></div><div class="hole-map-summary"><div><small>HOLE</small><b>${h}</b></div><div><small>PLAYING ROUTE</small><b>${yards} <i>YDS</i></b></div><div><small>PAR</small><b>${p}</b></div></div><div class="live-map-style-toggle" aria-label="Map style"><button class="${liveMapStyle==='street'?'on':''}" onclick="setLiveMapStyle('street')">Map</button><button class="${liveMapStyle==='satellite'?'on':''}" onclick="setLiveMapStyle('satellite')">Satellite</button></div><div class="shot-planner-controls"><button class="${shotPlannerEnabled?'on':''}" onclick="toggleShotPlanner()">◎ Planner ${shotPlannerEnabled?'On':'Off'}</button>${shotPlannerEnabled?'<button onclick="resetShotPlannerAim()">Reset Aim</button>':''}</div>${shotPlannerEnabled?'<div class="shot-planner-readout"><div><small id="plannerMode">SHOT PLAN</small><b><span id="plannerTargetYards">—</span> <i>YD</i></b><em>To target</em></div><div><small>THEN</small><b><span id="plannerRemainingYards">—</span> <i>YD</i></b><em>To green</em></div><div><small>CLUB</small><b id="plannerClub">—</b><em>Saved carry</em></div></div>':''}<div class="hole-edge-navigation" aria-label="Change hole"><button class="hole-edge-arrow previous" onclick="prev()" aria-label="Previous hole" ${h===1?'disabled':''}>‹</button><button class="hole-edge-arrow next" onclick="next()" aria-label="Next hole">›</button></div><div class="forward-label">FAIRWAY ROUTE · FORWARD</div><div class="map-wind-card"><small>WIND</small><span id="mapWindArrow" class="map-wind-arrow">↑</span><b id="mapWindSpeed">—</b><em id="mapWindLabel">Loading</em></div><div class="map-zoom-controls" aria-label="Map zoom controls"><button onclick="zoomLiveHoleMap(1)" aria-label="Zoom map in">+</button><button onclick="zoomLiveHoleMap(-1)" aria-label="Zoom map out">−</button></div><div class="hole-map-legend inline-legend"><span><i class="tee-dot"></i>Tee</span><span><i class="aim-dot"></i>Aim</span><span><i class="golfer-dot"></i>You</span><span><i class="green-dot"></i>Green</span></div><a class="hole-map-attribution" href="https://www.maptiler.com/copyright/" target="_blank" rel="noopener">© MapTiler</a></section>`;
}
function round(){
...
function initInlineHoleMap(green){
  ...
  L.polyline(route.map(point=>[point.lat,point.lng]),{color:'#fff',weight:4,opacity:shotPlannerEnabled ? .35 : .95}).addTo(inlineHoleMap);
  ...
  if(shotPlannerEnabled){
    shotPlannerGreen=green;const origin=shotPlannerOrigin(green),aim=shotPlannerAim(green),icon=L.divIcon({className:'shot-planner-marker',html:'<span>◎</span>',iconSize:[42,42],iconAnchor:[21,21]});
    inlinePlannerLines=[L.polyline([origin,aim],{color:'#f5cf68',weight:5,opacity:1}).addTo(inlineHoleMap),L.polyline([aim,green.center],{color:'#fff',weight:3,opacity:.9,dashArray:'8 9'}).addTo(inlineHoleMap)];
    inlinePlannerMarker=L.marker(aim,{icon,interactive:true,keyboard:false,zIndexOffset:1200}).addTo(inlineHoleMap);updateShotPlanner(green);
  }
...
function startLocation(green){
  ...
  locationWatch=navigator.geolocation.watchPosition(pos=>{
    ...
    const usePlannerTarget=shotPlannerEnabled&&golferIsNearHole(green),displayTarget=usePlannerTarget?shotPlannerAim(green):segment.target,targetYards=Math.round(distanceYards(here,displayTarget)),yardage=$('centerYards'),label=$('yardageTargetLabel');if(yardage)yardage.textContent=targetYards;if(label)label.textContent=usePlannerTarget?'Yards to Target':(segment.isGreen?'Yards to Hole':'Yards to Aim Point');
    updateClubSuggestion(targetYards,accuracyYards);updateInlineGolferPosition(here,green);if(shotPlannerEnabled)updateShotPlanner(green);orientInlineHoleMap(green,segment.origin,segment.target);loadWeather(here,segment.target,here);
