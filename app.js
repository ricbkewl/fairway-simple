...
function round(){ensureCurrentHolePar();const h=s.hole,p=s.pars[h-1],pt=parTotal(h),c=courseById(s.courseId),green=c?.greens?.[h-1];app.innerHTML=`<div class="row round-heading"><div><div class="brand small-brand">Agape Tumoutou Golfers</div><span class="small muted">${esc(s.course)}</span> <span id="syncStatus" class="sync-status ${navigator.onLine?'live':'waiting'}">${navigator.onLine?'Live':'Offline'}</span></div><button class="scorecard-button" onclick="openScorecard()"><span>▦</span> Live Scorecard <i>${s.players.length}</i></button></div><section class="hole hole-focus"><div class="hole-count"><span>HOLE</span><strong>${h}</strong><small>of ${s.holes}</small><i>PAR ${p}</i></div><h1>Enter your score</h1></section>${green?yardagePanel():`<div class="notice">No GPS markers for this hole. Add them from Map & Manage Courses.</div>`}${s.players.filter(x=>isMyPlayer(x)).map(x=>{const mine=isMyPlayer(x);return`<div class="card score ${mine?'my-score':''}"><div><b>${esc(x)}${mine?' (You)':''}</b><div class="small muted">Total ${total(x,h)} · ${rel(total(x,h)-pt)}</div></div>${mine?`<div class="stepper"><button onclick="changeScore('${encodeURIComponent(x)}',-1)">−</button><span>${scoreValue(x)||'–'}</span><button onclick="changeScore('${encodeURIComponent(x)}',1)">+</button></div>`:`<span class="locked-score">${scoreValue(x)||'–'} 🔒</span>`}<span class="green">${scoreValue(x)?rel(scoreValue(x)-p):''}</span></div>`}).join('')}<div class="row"><button class="secondary half" onclick="prev()">Previous</button><button class="primary fit" onclick="next()">${h===s.holes?'Finish':'Next Hole →'}</button></div>`;updateSyncIndicator();if(green)startLocation(green)}
