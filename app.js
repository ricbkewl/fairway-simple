...
function render(){save();document.querySelector('meta[name="theme-color"]')?.setAttribute('content',s.v==='round'?'#f7faf8':'#064c32');stopLocation();if(inlineHoleMap){inlineHoleMap.remove();inlineHoleMap=null;inlineGolferMarker=null;inlinePlannerMarker=null;inlinePlannerLines=[];inlinePlannerLabels=[];shotPlannerGreen=null}for(const previewMap of coursePreviewMaps){try{previewMap.remove()}catch{}}coursePreviewMaps=[];if(map){if(draft){const center=map.getCenter();draft.mapView={lat:center.lat,lng:center.lng,zoom:map.getZoom()}}map.remove();map=null}app.className='';({home,setup,pars,round,recap,coursesView,mapCourse,accountView,historyView,historyDetailView,clubsView,chatView,usersView,signupView,profileView,roundManageView}[s.v]||home)();if(!['home','signupView'].includes(s.v))bottomNav()}
