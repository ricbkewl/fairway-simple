...
function render(){save();stopLocation();if(map){if(draft){const center=map.getCenter();draft.mapView={lat:center.lat,lng:center.lng,zoom:map.getZoom()}}map.remove();map=null}app.className='';({home,setup,pars,round,recap,coursesView,mapCourse,accountView,historyView,historyDetailView,clubsView,chatView,usersView}[s.v]||home)();if(s.v!=='home')bottomNav()}
function appGuide(){return`<details class="app-guide"><summary><span>App Guide & About</span><b>＋</b></summary><div class="guide-body"><section class="founder-card"><img src="rick-kulon-profile.jpg" alt="Rick Kulon, creator of the Agape Tumoutou Golfers app"><div><small>CREATED FOR THE FELLOWSHIP</small><h2>Agape Tumoutou Golfers</h2><p>A shared golf companion created by Rick Kulon to make fellowship rounds easier, fairer and more connected.</p></div></section><h3>How to Play</h3><ol><li>Create a golfer account or sign in.</li><li>One golfer creates the round and shares its code or QR.</li><li>Other golfers join on their own phones.</li><li>Each golfer enters only their own score.</li><li>Use Live Scorecard to follow the group.</li></ol><h3>App Features</h3><ul><li>Shared course maps and live GPS yardages</li><li>Front, center and back green distances</li><li>Personal club suggestions</li><li>Protected individual scoring and live group scorecards</li><li>Round QR joining, private chat and unread alerts</li><li>Previous matches and offline score recovery</li></ul><div class="guide-update">Last updated August 27, 2026</div><div class="guide-contact"><p>Suggestions for improving the app are welcome.</p><a href="mailto:ricbkewl@gmail.com?subject=Agape%20Golf%20App%20Suggestion">✉ ricbkewl@gmail.com</a><a href="sms:+16074383208">✆ Text 607.438.3208</a></div></div></details>`}
...
async function signInFromHome(event){
  ...
  if(s.ownerUserId&&s.ownerUserId!==currentUser.id)s={...roundDefault};
  await Promise.all([loadAdminRole(),loadClubDistances()]);render();return true;
}
...
async function signOutAdmin(){await stopRoundRealtime();await db.auth.signOut();delete localStorage.atgPendingJoinCode;currentUser=null;adminRole=null;historyRounds=[];historyDetail=null;registeredGolfers=[];registeredGolfersError='';clubDistances={};clubProfileError='';s={...roundDefault};render()}
...
async function openRegisteredGolfers(){
...
function usersView(){
  ...
  app.innerHTML=`<button class="back" onclick="accountAction()">← Account</button><div class="row users-heading"><div><h1>Registered Golfers</h1><p class="muted">Private account directory · Super Admin only</p></div>${!registeredGolfersLoading?'<button class="locate" onclick="openRegisteredGolfers()">Refresh</button>':''}</div>${registeredGolfersLoading?'<div class="history-loading">Loading registered golfers…</div>':''}${registeredGolfersError?`<div class="error-notice">${esc(registeredGolfersError)}</div>`:''}${!registeredGolfersLoading&&!registeredGolfersError?`<div class="users-count">${registeredGolfers.length} registered ${registeredGolfers.length===1?'golfer':'golfers'}</div>`:''}${registeredGolfers.map(user=>`<article class="user-card"><div class="user-avatar">${esc((user.email||'?').charAt(0).toUpperCase())}</div><div class="user-details"><b>${esc(user.email||'No email')}</b><span>Signed up ${esc(formatAccountDate(user.created_at))}</span><span>Last sign-in ${esc(formatAccountDate(user.last_sign_in_at))}</span></div><span class="user-role ${user.role?'admin':''}">${esc(user.role?user.role.replace('_',' '):'golfer')}</span></article>`).join('')}<div class="notice">This directory is private. Course admins and regular golfers cannot access it. Passwords are never displayed.</div>`;
}
