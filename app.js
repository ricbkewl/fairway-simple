...
function orientInlineHoleMap(green,origin=null,target=null){
  ...
  const segment=activeRouteSegment(null,green),start=origin||segment?.origin||green.tee,end=target||segment?.target||green.center,container=$('liveHoleMap'),bearing=bearingDegrees(start,end);if(container){container.dataset.forwardBearing=String(bearing);container.style.transform=`rotate(${-bearing}deg)`}
}
...
function initInlineHoleMap(green){
  ...
  const route=holeRoute(green),points=[...route,green.front,green.back].filter(Boolean);inlineHoleMap.fitBounds(points.map(point=>[point.lat,point.lng]),{padding:[72,48],maxZoom:18});inlineHoleMap.setZoom(inlineHoleMap.getZoom()-.75,{animate:false});
  L.polyline(route.map(point=>[point.lat,point.lng]),{color:'#fff',weight:4,opacity:.95}).addTo(inlineHoleMap);
  for(let i=1;i<route.length;i++){const start=route[i-1],end=route[i],mid={lat:(start.lat+end.lat)/2,lng:(start.lng+end.lng)/2},bearing=bearingDegrees(start,end),icon=L.divIcon({className:'route-arrow-icon',html:`<span style="transform:rotate(${bearing}deg)">▲</span>`,iconSize:[24,24],iconAnchor:[12,12]});L.marker(mid,{icon,interactive:false}).addTo(inlineHoleMap)}
...
function startLocation(green){
  ...
  locationWatch=navigator.geolocation.watchPosition(pos=>{
    ...
    const here={lat:pos.coords.latitude,lng:pos.coords.longitude},segment=activeRouteSegment(here,green);lastKnownPosition=here;if(!segment)return;
    const targetYards=Math.round(distanceYards(here,segment.target)),yardage=$('centerYards'),label=$('yardageTargetLabel');if(yardage)yardage.textContent=targetYards;if(label)label.textContent=segment.isGreen?'Yards to Hole':'Yards to Aim Point';
    updateClubSuggestion(targetYards,accuracyYards);updateInlineGolferPosition(here,green);orientInlineHoleMap(green,segment.origin,segment.target);loadWeather(here,segment.target,here);
