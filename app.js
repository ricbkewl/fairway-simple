...
function createGoogleHtmlOverlay(position,className,html){
  class AtgOverlay extends google.maps.OverlayView{
    ...
    draw(){const projection=this.getProjection();if(!projection)return;const point=projection.fromLatLngToDivPixel(this.position);if(!point)return;this.div.style.left=`${point.x}px`;this.div.style.top=`${point.y}px`}
