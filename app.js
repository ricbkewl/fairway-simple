...
let lastKnownPosition=null,inlineHoleMap=null,inlineGolferMarker=null;
...
function mappedHoleDistance(green){return green?.tee&&green?.center?Math.round(distanceYards(green.tee,green.center)):null}
function liveHoleMapPanel(green,h,p){
  ...
  return`<section class="live-hole-map"><div class="live-map-viewport"><div id="liveHoleMap" aria-label="Forward-facing satellite view of Hole ${h}"></div></div><div class="hole-map-summary"><div><small>HOLE</small><b>${h}</b></div><div><small>TEE TO CENTER</small><b>${yards} <i>YDS</i></b></div><div><small>PAR</small><b>${p}</b></div></div><div class="forward-label">TEE → GREEN · FORWARD</div><div class="map-wind-card"><small>WIND</small><span id="mapWindArrow" class="map-wind-arrow">↑</span><b id="mapWindSpeed">—</b><em id="mapWindLabel">Loading</em></div><div class="hole-map-legend inline-legend"><span><i class="tee-dot"></i>Tee</span><span><i class="golfer-dot"></i>You</span><span><i class="green-dot"></i>Green</span></div><a class="hole-map-attribution" href="https://www.maptiler.com/copyright/" target="_blank" rel="noopener">© MapTiler</a></section>`;
}
function round(){
  ...
  app.innerHTML=`<div class="row round-heading compact-round-heading"><div class="brand small-brand">Agape Tumoutou Golfers</div>${s.sharedRoundId?'<button class="round-menu-button" onclick="openRoundManagement()" aria-label="Round options">•••</button>':''}</div><div class="round-course-title">${esc(s.course)} <span id="syncStatus" class="sync-status ${navigator.onLine?'live':'waiting'}">${navigator.onLine?'Live':'Offline'}</span></div>${green?liveHoleMapPanel(green,h,p)+yardagePanel():`<div class="notice">No GPS markers for this hole. Add them from Map & Manage Courses.</div>`}${s.players.filter(x=>isMyPlayer(x)).map(x=>{const mine=isMyPlayer(x);return`<section class="score-and-scorecard"><div class="card score ${mine?'my-score':''}"><div><b>${esc(x)}${mine?' (You)':''}</b><div class="current-hole-reference">Scoring Hole ${h} · Par ${p}</div><div class="small muted">Total ${total(x,h)} · ${rel(total(x,h)-pt)}</div></div>${mine?`<div class="stepper"><button onclick="changeScore('${encodeURIComponent(x)}',-1)">−</button><span>${scoreValue(x)||'–'}</span><button onclick="changeScore('${encodeURIComponent(x)}',1)">+</button></div>`:`<span class="locked-score">${scoreValue(x)||'–'} 🔒</span>`}<span class="green">${scoreValue(x)?rel(scoreValue(x)-p):''}</span></div><button class="scorecard-button player-scorecard-button" onclick="openScorecard()"><span>▦</span> Live Scorecard <i>${s.players.length}</i></button></section>`}).join('')}<div class="row"><button class="secondary half" onclick="prev()">Previous</button><button class="primary fit" onclick="next()">${h===s.holes?'Finish':'Next Hole →'}</button></div>`;
  updateSyncIndicator();if(green){initInlineHoleMap(green);loadWeather(green.tee||green.center,green.center,green.tee);startLocation(green)}
}
function yardagePanel(){return`<section class="gps-card"><div class="gps-signal-row top-gps-signal"><div><b>GPS</b><div id="gpsStatus" class="small muted">Locating…</div></div><button class="locate" onclick="refreshLocation()">Refresh</button></div><div class="club-suggestion featured-club"><div class="club-symbol">⛳</div><div><small>Suggested Club</small><b id="clubSuggestion">—</b><span id="clubSuggestionNote">Waiting for an accurate GPS signal</span></div></div><div class="yardages"><div><span id="frontYards">–</span><small>Front</small></div><div class="center-yard"><span id="centerYards">–</span><small>Center</small></div><div><span id="backYards">–</span><small>Back</small></div></div><section id="weatherPanel" class="weather-panel"><div class="weather-icon">◌</div><div><b>Weather & Wind</b><span id="weatherSummary">Loading course weather…</span><small id="windEffect">Arrow on the map is relative to the green being straight ahead</small><a href="https://open-meteo.com/" target="_blank" rel="noopener">Weather by Open-Meteo</a></div></section></section>`}
function orientInlineHoleMap(green){
  if(!inlineHoleMap||!green?.tee||!green?.center)return;
  const container=$('liveHoleMap'),bearing=bearingDegrees(green.tee,green.center);if(container)container.style.transform=`rotate(${-bearing}deg)`;
}
function initInlineHoleMap(green){
  const container=$('liveHoleMap');if(!container||!green?.tee||!green?.center||!window.L)return;
  inlineHoleMap=L.map(container,{zoomControl:false,zoomSnap:.25,dragging:false,scrollWheelZoom:false,doubleClickZoom:false,boxZoom:false,keyboard:false,touchZoom:false,attributionControl:false});
  const useStreetAttribution=()=>{const label=document.querySelector('.forward-label'),credit=document.querySelector('.hole-map-attribution');if(label)label.textContent='SATELLITE UNAVAILABLE · STREET VIEW · FORWARD';if(credit){credit.textContent='© OpenStreetMap';credit.href='https://www.openstreetmap.org/copyright/'}};
  if(MAPTILER_API_KEY)addSatelliteLayer(inlineHoleMap,useStreetAttribution);else{addStreetLayer(inlineHoleMap);useStreetAttribution()}
  const points=[green.tee,green.front,green.center,green.back].filter(Boolean);inlineHoleMap.fitBounds(points.map(point=>[point.lat,point.lng]),{padding:[72,48],maxZoom:18});inlineHoleMap.setZoom(inlineHoleMap.getZoom()-.75,{animate:false});
  L.polyline([[green.tee.lat,green.tee.lng],[green.center.lat,green.center.lng]],{color:'#fff',weight:4,opacity:.95}).addTo(inlineHoleMap);
  ...
  setTimeout(()=>orientInlineHoleMap(green),120);
}
...
function startLocation(green){
  ...
  locationWatch=navigator.geolocation.watchPosition(pos=>{
    ...
    updateClubSuggestion(yards.center,accuracyYards);updateInlineGolferPosition(here,green);loadWeather(here,green.center,green.tee);
...
function showWeather(weather,here,target,tee){
  ...
  if(!weather){summary.textContent='Weather temporarily unavailable';effect.textContent='GPS yardages still work normally';updateMapWind(null,tee||here,target);return}
  ...
  effect.textContent=windEffectText(weather,tee||here,target);updateMapWind(weather,tee||here,target);
}
