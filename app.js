...
function liveHoleMapPanel(green,h,p){
  ...
  return`<section class="live-hole-map"><div class="live-map-viewport"><div id="liveHoleMap" aria-label="Forward-facing course view of Hole ${h}"></div></div><div class="hole-map-summary"><div><small>HOLE</small><b>${h}</b></div><div><small>PLAYING ROUTE</small><b>${yards} <i>YDS</i></b></div><div><small>PAR</small><b>${p}</b></div></div><div class="live-map-style-toggle" aria-label="Map style"><button class="${liveMapStyle==='street'?'on':''}" onclick="setLiveMapStyle('street')">Map</button><button class="${liveMapStyle==='satellite'?'on':''}" onclick="setLiveMapStyle('satellite')">Satellite</button></div><div class="forward-label">FAIRWAY ROUTE · FORWARD</div><div class="map-wind-card"><small>WIND</small><span id="mapWindArrow" class="map-wind-arrow">↑</span><b id="mapWindSpeed">—</b><em id="mapWindLabel">Loading</em></div><div class="map-zoom-controls" aria-label="Map zoom controls"><button onclick="zoomLiveHoleMap(1)" aria-label="Zoom map in">+</button><button onclick="zoomLiveHoleMap(-1)" aria-label="Zoom map out">−</button></div><div class="hole-map-legend inline-legend"><span><i class="tee-dot"></i>Tee</span><span><i class="aim-dot"></i>Aim</span><span><i class="golfer-dot"></i>You</span><span><i class="green-dot"></i>Green</span></div><a class="hole-map-attribution" href="https://www.maptiler.com/copyright/" target="_blank" rel="noopener">© MapTiler</a></section>`;
}
function round(){
