...
function render(){save();stopLocation();if(map){if(draft){const center=map.getCenter();draft.mapView={lat:center.lat,lng:center.lng,zoom:map.getZoom()}}map.remove();map=null}({home,setup,pars,round,recap,coursesView,mapCourse}[s.v]||home)()}
function home(){app.innerHTML=`<section class="hero"><img src="agape-golf-logo.png" alt="Agape Tumoutou Golfers logo" class="landing-logo"><div class="brand">Agape Tumoutou Golfers</div><h1>Golf scoring and GPS,<br>made simple.</h1><p class="muted">One shared course library for the whole fellowship.</p></section>${cloudLoading?'<div class="notice">Loading shared courses…</div>':''}${cloudError?`<div class="error-notice">${esc(cloudError)}</div>`:''}<button class="primary" onclick="start()" ${cloudLoading?'disabled':''}>Start a Round</button><button class="secondary" onclick="openCourses()">${adminRole?'Map & Manage Courses':'View Shared Courses'}</button><div class="account-bar">${currentUser?`<span>${adminRole?esc(adminRole.replace('_',' ')):'Signed in'}</span><button class="back" onclick="signOutAdmin()">Sign out</button>`:`<span class="small muted">Course managers only</span><button class="back" onclick="signInAdmin()">Admin sign in</button>`}</div>`}
