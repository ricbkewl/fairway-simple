...
let lastKnownPosition=null,lastGpsAccuracyYards=null,inlineHoleMap=null,inlineGolferMarker=null,inlineHoleGreen=null,inlineViewResetting=false;
...
function liveHoleMapPanel(green,h,p){
  ...
  return`<section class="live-hole-map ${shotPlannerEnabled?'planner-on':''}"><div class="live-map-viewport"><div id="liveHoleMap" aria-label="Forward-facing course view of Hole ${h}"></div></div><div class="hole-map-summary round-map-summary"><div><small>HOLE</small><b>${h}</b></div><div class="hole-distance-summary"><small>DISTANCE</small><b>${yards} <i>YDS</i></b></div><div><small>PAR</small><b>${p}</b></div><div class="route-remaining-summary"><small>ROUTE REMAINING</small><b><span id="centerYards">${yards}</span> <i>YDS</i></b><span id="yardageTargetLabel" class="visually-hidden">Route Remaining</span></div><div class="round-qr-summary"><button onclick="showRoundQr()" aria-label="Show QR code for the current round" title="Show current round QR code"><img src="icon-192.png" alt="ATG"></button></div></div><div id="gpsStatus" class="visually-hidden">Locating…</div><button id="mapRecenterButton" class="map-recenter-button hidden" onclick="resetLiveHoleView()" aria-label="Restore the complete hole view" title="Restore hole view"><span></span></button><div class="live-map-style-toggle" aria-label="Map style"><button class="${liveMapStyle==='street'?'on':''}" onclick="setLiveMapStyle('street')">Map</button><button class="${liveMapStyle==='satellite'?'on':''}" onclick="setLiveMapStyle('satellite')">Satellite</button></div><div class="shot-planner-controls"><button class="${shotPlannerEnabled?'on':''}" onclick="toggleShotPlanner()">${shotPlannerEnabled?'Hide<br>Planner':'Show<br>Planner'}</button></div><div class="hole-edge-navigation" aria-label="Change hole"><button class="hole-edge-arrow previous" onclick="prev()" aria-label="Previous hole" ${h===1?'disabled':''}>‹</button><button class="hole-edge-arrow next" onclick="next()" aria-label="Next hole">›</button></div><div class="forward-label">SHOT PLANNER · FORWARD</div><div class="map-wind-card"><div class="map-weather-temperature"><span id="mapWeatherIcon">◌</span><b id="mapTemperature">—°</b></div><small>WIND</small><span id="mapWindArrow" class="map-wind-arrow">↑</span><b id="mapWindSpeed">—</b><em id="mapWindLabel">Loading</em></div><div class="map-zoom-controls" aria-label="Map zoom controls"><button onclick="zoomLiveHoleMap(1)" aria-label="Zoom map in">+</button><button onclick="zoomLiveHoleMap(-1)" aria-label="Zoom map out">−</button></div><div class="hole-map-legend inline-legend"><span><i class="tee-dot"></i>Tee</span><span><i class="aim-dot"></i>Aim</span><span><i class="golfer-dot"></i>You</span><span><i class="green-dot"></i>Green</span></div><a class="hole-map-attribution" href="https://www.maptiler.com/copyright/" target="_blank" rel="noopener">© MapTiler</a></section>`;
}
function round(){
...
function showMapRecenterButton(){if(!inlineViewResetting)$('mapRecenterButton')?.classList.remove('hidden')}
function fitLiveHoleView(green){
  if(!inlineHoleMap||!green?.tee||!green?.center)return;
  inlineViewResetting=true;inlineHoleGreen=green;$('mapRecenterButton')?.classList.add('hidden');
  const points=[...holeRoute(green),green.front,green.back].filter(Boolean);
  inlineHoleMap.fitBounds(points.map(point=>[point.lat,point.lng]),{padding:[72,48],maxZoom:22,animate:false});
  inlineHoleMap.setZoom(inlineHoleMap.getZoom()-1,{animate:false});
...
function courseLibraryCard(course,index){const point=coursePreviewPoint(course);return`<article class="course-library-card" data-course-name="${esc(course.name.toLowerCase())}"><div class="course-preview course-start-target" role="button" tabindex="0" onclick="startCourseFromLibrary(${index})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();startCourseFromLibrary(${index})}">${point?`<div id="coursePreview${index}" class="course-preview-map" data-lat="${point.lat}" data-lng="${point.lng}" aria-label="Satellite preview of ${esc(course.name)}"></div>`:''}<div class="course-preview-placeholder"><span>⛳</span><small>${point?'Loading satellite preview':'Map this course for a preview'}</small></div>${point?'<a href="https://www.maptiler.com/copyright/" target="_blank" rel="noopener" onclick="event.stopPropagation()">© MapTiler</a>':''}</div><div class="course-card-info"><div><small>${course.holes} HOLES · ${mappedCount(course)} MAPPED</small><button class="course-name-start" onclick="startCourseFromLibrary(${index})">${esc(course.name)}</button><span>Tap the map or course name to play</span></div>${adminRole?`<button onclick="editCourse(${index})">Edit</button>`:'<i>›</i>'}</div></article>`}
async function startCourseFromLibrary(index){const course=courses[index];if(!course)return;if(!confirm(`Would you like to start a new game at ${course.name}?`))return;const courseId=course.id;await start();if(s.v!=='setup')return;chooseCourse(courseId)}
