...
function yardagePanel(){return`<section class="gps-card"><div class="gps-signal-row top-gps-signal gps-compact-row"><div class="gps-accuracy"><b>GPS</b><div id="gpsStatus" class="small muted">Locating…</div></div><div class="hole-yardage-compact"><small>Yards to Hole</small><b id="centerYards">–</b><em>yd</em></div><button class="locate" onclick="refreshLocation()">Refresh</button></div><div class="club-suggestion featured-club"><div class="club-symbol">⛳</div><div><small>Suggested Club</small><b id="clubSuggestion">—</b><span id="clubSuggestionNote">Waiting for an accurate GPS signal</span></div></div></section>`}
function orientInlineHoleMap(green){
  if(!inlineHoleMap||!green?.tee||!green?.center)return;
  const container=$('liveHoleMap'),bearing=bearingDegrees(green.tee,green.center);if(container){container.dataset.forwardBearing=String(bearing);container.style.transform=`rotate(${-bearing}deg)`}
}
...
function enableForwardMapDragging(){
  ...
  const pointers=new Map();let dragPointer=null;
  const resetRemaining=()=>{if(pointers.size===1){const [id]=pointers.keys();dragPointer=id}else dragPointer=null};
  viewport.addEventListener('pointerdown',event=>{if(event.pointerType==='mouse'&&event.button!==0)return;pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});if(pointers.size===1)dragPointer=event.pointerId;else dragPointer=null;try{viewport.setPointerCapture(event.pointerId)}catch{}});
  viewport.addEventListener('pointermove',event=>{const prior=pointers.get(event.pointerId);if(!prior)return;pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});if(pointers.size!==1||dragPointer!==event.pointerId||!inlineHoleMap)return;const dx=event.clientX-prior.x,dy=event.clientY-prior.y;if(!dx&&!dy)return;const angle=Number(container.dataset.forwardBearing||0)*Math.PI/180,localX=Math.cos(angle)*dx-Math.sin(angle)*dy,localY=Math.sin(angle)*dx+Math.cos(angle)*dy;inlineHoleMap.panBy([-localX,-localY],{animate:false})});
...
function showWeather(weather,here,target,tee){
  const summary=$('weatherSummary'),effect=$('windEffect');
  if(!weather){if(summary)summary.textContent='Weather temporarily unavailable';if(effect)effect.textContent='GPS yardages still work normally';updateMapWind(null,tee||here,target);return}
