...
let lastKnownPosition=null,holePreviewMap=null;
...
function appGuide(){return`<details class="app-guide"><summary><span>App Guide & About</span><b>＋</b></summary><div class="guide-body"><section class="founder-card"><img src="rick-kulon-profile.jpg" alt="Rick Kulon, creator of the Agape Tumoutou Golfers app"><div><small>CREATED FOR THE FELLOWSHIP</small><h2>Agape Tumoutou Golfers</h2><p>A shared golf companion created by Rick Kulon to make fellowship rounds easier, fairer and more connected.</p></div></section><h3>How to Get Started</h3><ol><li>Sign up with your first name, last name, email and phone number.</li><li><b>Open your email and tap the verification link.</b> You cannot sign in until your email is verified.</li><li>Return to the app and sign in with your email and password.</li><li>One golfer creates a round and shares its six-character code or QR.</li><li>Other golfers join on their own phones and enter only their own scores.</li></ol><h3>Save the App to Your Home Screen</h3><div class="install-guide"><section><h4>iPhone or iPad</h4><ol><li>Open the app in <b>Safari</b>.</li><li>Tap the <b>Share</b> button at the bottom of Safari.</li><li>Scroll down and tap <b>Add to Home Screen</b>.</li><li>Tap <b>Add</b>.</li></ol></section><section><h4>Android</h4><ol><li>Open the app in <b>Chrome</b>.</li><li>Tap the <b>three-dot menu</b>.</li><li>Tap <b>Add to Home screen</b> or <b>Install app</b>.</li><li>Tap <b>Add</b> or <b>Install</b>.</li></ol></section></div><div class="notice"><b>Tip:</b> If the link opens inside Messages, Facebook or another app, first choose <b>Open in Safari</b> or <b>Open in Chrome</b>. The Home Screen option may not appear inside an in-app browser.</div><h3>My Clubs & Suggested Club</h3><p>Open <b>Account → My Clubs & Distances</b> and enter the distance you normally carry each club. The app compares your personal distances with the GPS distance to the center of the green and makes the Suggested Club easier to see during play.</p><div class="notice"><b>Important:</b> A club suggestion is a starting point. Wind, elevation, temperature, lie, hazards, swing conditions and rollout can change the correct choice.</div><h3>Current App Features</h3><ul><li>Individual verified golfer accounts, remembered sign-in and profile pictures</li><li>Protected scoring: each golfer edits only their own score</li><li>Scores begin at par with simple plus and minus controls</li><li>Live group scorecards, previous-match history and scorecard sharing</li><li>Six-character round codes, join links, generated QR codes and a QR scanner</li><li>Private round chat with unread-message alerts</li><li>Front, center and back green GPS yardages with accuracy warnings</li><li>Personal club carry profiles and prominent Suggested Club guidance</li><li>Live temperature, conditions, wind direction and wind effect</li><li>Shared course maps with course-name and address search</li><li>Street and satellite course mapping with tee and green markers</li><li>On-demand bird’s-eye satellite views of mapped holes</li><li>Secure host controls to end, reopen and manage a round</li><li>Administrator course tools and a private Super Admin player directory</li><li>Current-round recovery, offline score queue and automatic synchronization</li><li>Custom iPhone and Android Home Screen icons</li></ul><div class="guide-update">Last updated August 27, 2026</div><div class="guide-contact"><p>Suggestions for improving the app are welcome.</p><a href="mailto:ricbkewl@gmail.com?subject=Agape%20Golf%20App%20Suggestion">✉ ricbkewl@gmail.com</a><a href="sms:+16074383208">✆ Text 607.438.3208</a></div></div></details>`}
...
function updateClubSuggestion(centerYards,accuracyYards){
  ...
  if(accuracyYards>50){title.textContent='Waiting for GPS';note.textContent='A club will appear when the GPS signal becomes accurate.';return}
  if(centerYards>650){title.textContent='Move Closer';note.textContent='A club will appear when you are near the mapped hole.';return}
...
function drawScorecardImage(data){
...
async function shareScorecardData(data){
  if(!data)return;const canvas=drawScorecardImage(data),blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));
...
function round(){
  ...
  ensureCurrentHolePar();const h=s.hole,p=s.pars[h-1],pt=parTotal(h),c=courseById(s.courseId),green=c?.greens?.[h-1];
  app.innerHTML=`<div class="row round-heading"><div><div class="brand small-brand">Agape Tumoutou Golfers</div><span class="small muted">${esc(s.course)}</span> <span id="syncStatus" class="sync-status ${navigator.onLine?'live':'waiting'}">${navigator.onLine?'Live':'Offline'}</span></div><div class="round-heading-actions"><button class="scorecard-button" onclick="openScorecard()"><span>▦</span> Live Scorecard <i>${s.players.length}</i></button>${s.sharedRoundId?'<button class="round-menu-button" onclick="openRoundManagement()" aria-label="Round options">•••</button>':''}</div></div><section class="hole hole-focus"><div class="hole-count"><span>HOLE</span><strong>${h}</strong><small>of ${s.holes}</small><i>PAR ${p}</i></div><h1>Enter your score</h1></section>${green?yardagePanel()+holePreviewControl(green):`<div class="notice">No GPS markers for this hole. Add them from Map & Manage Courses.</div>`}${s.players.filter(x=>isMyPlayer(x)).map(x=>{const mine=isMyPlayer(x);return`<div class="card score ${mine?'my-score':''}"><div><b>${esc(x)}${mine?' (You)':''}</b><div class="current-hole-reference">Scoring Hole ${h} · Par ${p}</div><div class="small muted">Total ${total(x,h)} · ${rel(total(x,h)-pt)}</div></div>${mine?`<div class="stepper"><button onclick="changeScore('${encodeURIComponent(x)}',-1)">−</button><span>${scoreValue(x)||'–'}</span><button onclick="changeScore('${encodeURIComponent(x)}',1)">+</button></div>`:`<span class="locked-score">${scoreValue(x)||'–'} 🔒</span>`}<span class="green">${scoreValue(x)?rel(scoreValue(x)-p):''}</span></div>`}).join('')}<div class="row"><button class="secondary half" onclick="prev()">Previous</button><button class="primary fit" onclick="next()">${h===s.holes?'Finish':'Next Hole →'}</button></div>`;
  updateSyncIndicator();if(green)startLocation(green);
}
function yardagePanel(){return`<section class="gps-card"><div class="gps-signal-row top-gps-signal"><div><b>GPS Status</b><div id="gpsStatus" class="small muted">Requesting your location…</div></div><button class="locate" onclick="refreshLocation()">Refresh</button></div><div class="club-suggestion featured-club"><div class="club-symbol">⛳</div><div><small>Suggested Club</small><b id="clubSuggestion">Waiting for GPS</b><span id="clubSuggestionNote">Based on your personal carry distances</span></div></div><div class="yardages"><div><span id="frontYards">–</span><small>Front</small></div><div class="center-yard"><span id="centerYards">–</span><small>Center</small></div><div><span id="backYards">–</span><small>Back</small></div></div><section id="weatherPanel" class="weather-panel"><div class="weather-icon">◌</div><div><b>Weather & Wind</b><span id="weatherSummary">Waiting for your location…</span><small id="windEffect">Wind effect will appear here</small><a href="https://open-meteo.com/" target="_blank" rel="noopener">Weather by Open-Meteo</a></div></section></section>`}
function holePreviewControl(green){const ready=Boolean(green?.tee&&green?.center&&MAPTILER_API_KEY);return`<button class="hole-preview-button" onclick="openHolePreview()" ${ready?'':'disabled'}><span>◈</span><div><b>View Hole</b><small>${!MAPTILER_API_KEY?'Satellite setup pending':!green?.tee?'Tee marker needed':'Bird’s-eye satellite preview'}</small></div><i>→</i></button>`}
function closeHolePreview(){if(holePreviewMap){holePreviewMap.remove();holePreviewMap=null}document.querySelector('.hole-preview-overlay')?.remove()}
function openHolePreview(){
  const green=courseById(s.courseId)?.greens?.[s.hole-1];if(!MAPTILER_API_KEY){alert('Satellite setup is not active yet.');return}if(!green?.tee||!green?.center){alert('An administrator needs to add the tee and center markers for this hole.');return}const nearby=lastKnownPosition&&distanceYards(lastKnownPosition,green.center)<3000;
  closeHolePreview();const overlay=document.createElement('div');overlay.className='hole-preview-overlay';overlay.onclick=event=>{if(event.target===overlay)closeHolePreview()};overlay.innerHTML=`<section class="hole-preview-modal"><button class="qr-close" onclick="closeHolePreview()">×</button><div class="hole-preview-title"><span>HOLE ${s.hole}</span><h2>${esc(s.course)}</h2><small>Par ${s.pars[s.hole-1]} · North-up bird’s-eye view</small></div><div id="holePreviewMap" aria-label="Satellite view of hole ${s.hole}"></div><div class="hole-map-legend"><span><i class="tee-dot"></i>Tee</span>${nearby?'<span><i class="golfer-dot"></i>You</span>':''}<span><i class="green-dot"></i>Green</span></div><button class="secondary" onclick="closeHolePreview()">Close</button></section>`;document.body.appendChild(overlay);
  holePreviewMap=L.map('holePreviewMap',{zoomControl:false,dragging:false,scrollWheelZoom:false,doubleClickZoom:false,boxZoom:false,keyboard:false,touchZoom:false,attributionControl:true});addSatelliteLayer(holePreviewMap,()=>{const subtitle=document.querySelector('.hole-preview-title small');if(subtitle)subtitle.textContent='Satellite unavailable · Street fallback shown'});
  const points=[green.tee,green.front,green.center,green.back].filter(Boolean);if(nearby)points.push(lastKnownPosition);holePreviewMap.fitBounds(points.map(p=>[p.lat,p.lng]),{padding:[35,35],maxZoom:18});
  L.circleMarker(green.tee,{radius:9,color:'#fff',weight:3,fillColor:'#d8a93e',fillOpacity:1}).addTo(holePreviewMap).bindTooltip('Tee');L.circleMarker(green.center,{radius:10,color:'#fff',weight:3,fillColor:'#176b45',fillOpacity:1}).addTo(holePreviewMap).bindTooltip('Green');L.polyline([[green.tee.lat,green.tee.lng],[green.center.lat,green.center.lng]],{color:'#fff',weight:3,dashArray:'8 8',opacity:.9}).addTo(holePreviewMap);if(nearby)L.circleMarker(lastKnownPosition,{radius:9,color:'#fff',weight:3,fillColor:'#2476d1',fillOpacity:1}).addTo(holePreviewMap).bindTooltip('You');
}
function startLocation(green){
  ...
  locationWatch=navigator.geolocation.watchPosition(pos=>{
    const accuracyYards=Math.round(pos.coords.accuracy*1.094),status=$('gpsStatus');status.textContent=`Accuracy ±${accuracyYards} yd`;status.classList.toggle('gps-warning',accuracyYards>50);
    const here={lat:pos.coords.latitude,lng:pos.coords.longitude},yards={};lastKnownPosition=here;
    ['front','center','back'].forEach(k=>{const el=$(k+'Yards');yards[k]=green[k]?Math.round(distanceYards(here,green[k])):null;if(el)el.textContent=yards[k]??'–'});
    updateClubSuggestion(yards.center,accuracyYards);loadWeather(here,green.center);
...
function showWeather(weather,here,target){
  const summary=$('weatherSummary'),effect=$('windEffect');if(!summary||!effect)return;
  if(!weather){summary.textContent='Weather temporarily unavailable';effect.textContent='GPS yardages still work normally';return}
  summary.textContent=`${Math.round(weather.temperature_2m)}°F · ${weatherDescription(weather.weather_code)} · Wind ${Math.round(weather.wind_speed_10m)} mph ${compassDirection(weather.wind_direction_10m)}`;
  effect.textContent=windEffectText(weather,here,target);
}
async function loadWeather(here,target,force=false){
  const key=`${here.lat.toFixed(2)},${here.lng.toFixed(2)}`,cached=weatherCache[key];
  if(!force&&cached&&Date.now()-cached.savedAt<WEATHER_CACHE_MS){currentWeather=cached.data;showWeather(currentWeather,here,target);return}
  if(weatherLoading){if(currentWeather)showWeather(currentWeather,here,target);return}
  weatherLoading=true;
  try{
    const url=`https://api.open-meteo.com/v1/forecast?latitude=${encodeURIComponent(here.lat)}&longitude=${encodeURIComponent(here.lng)}&current=temperature_2m,apparent_temperature,weather_code,wind_speed_10m,wind_direction_10m,wind_gusts_10m&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=auto`;
    ...
    weatherCache[key]={savedAt:Date.now(),data:currentWeather};localStorage.atgWeatherCache=JSON.stringify(weatherCache);showWeather(currentWeather,here,target);
  }catch{showWeather(null,here,target)}finally{weatherLoading=false}
}
...
function newCourse(){if(!adminRole){alert('Administrator sign-in required.');return}const name=prompt('Course name:');if(!name?.trim())return;const holes=confirm('Does this course have 18 holes?\nChoose Cancel for 9 holes.')?18:9;draft={id:crypto.randomUUID(),isNew:true,name:name.trim(),holes,pars:Array(holes).fill(4),greens:Array.from({length:holes},()=>({tee:null,front:null,center:null,back:null})),mapHole:1,target:'center'};s.v='mapCourse';render()}
...
function mapCourse(){if(!adminRole){s.v='coursesView';render();return}const h=draft.mapHole,g=draft.greens[h-1],satelliteReady=Boolean(MAPTILER_API_KEY);draft.mapStyle=draft.mapStyle||'street';app.innerHTML=`<button class="back" onclick="cancelMapping()">← Courses</button><h1>${esc(draft.name)}</h1><div class="row map-toolbar"><button class="back" onclick="mapPrev()">←</button><b>Hole ${h} of ${draft.holes}</b><button class="back" onclick="mapNext()">→</button></div><div class="row"><span>Par</span><div class="stepper"><button onclick="draft.pars[${h-1}]=Math.max(3,draft.pars[${h-1}]-1);render()">−</button><span>${draft.pars[h-1]}</span><button onclick="draft.pars[${h-1}]=Math.min(6,draft.pars[${h-1}]+1);render()">+</button></div></div><p class="muted small">Choose Tee or a green marker, then tap its exact position on the map.</p><div class="marker-tabs four-tabs">${['tee','front','center','back'].map(k=>`<button class="marker-tab ${draft.target===k?'on':''} ${g[k]?'set':''}" onclick="draft.target='${k}';render()">${k[0].toUpperCase()+k.slice(1)} ${g[k]?'✓':''}</button>`).join('')}</div><form class="course-search" onsubmit="event.preventDefault();searchCourseAddress()"><label for="courseSearch">Find course by name or address</label><div class="search-row"><input id="courseSearch" autocomplete="street-address" placeholder="Sierra Lakes Golf Club or street address"><button id="courseSearchButton" type="submit">Search</button></div></form><div id="courseSearchResults" class="search-results"></div><div class="search-divider"><span>or</span></div><button class="secondary locate-map" onclick="centerOnMe()">Use My Location to Find Course</button><div class="map-layer-toggle"><button class="${draft.mapStyle==='street'?'on':''}" onclick="setMapStyle('street')">Street</button><button class="${draft.mapStyle==='satellite'?'on':''}" onclick="setMapStyle('satellite')" ${satelliteReady?'':'disabled'}>Satellite</button></div>${satelliteReady?'':'<div class="satellite-key-note">Satellite view is ready for your MapTiler API key.</div>'}<div id="courseMap" aria-label="Course mapping map"></div><div id="mapMessage" class="notice">Tap the map to set the ${draft.target} of hole ${h}.</div><div class="marker-clear-actions"><button class="secondary" onclick="clearMarker()">Clear ${draft.target} marker</button><button class="secondary clear-all-markers" onclick="clearAllMarkers()">Clear All Markers · Hole ${h}</button></div><button id="saveCourseButton" class="primary" onclick="saveMappedCourse()">Save Shared Course</button>`;setTimeout(initMap,0)}
...
function initMap(){
  const g=draft.greens[draft.mapHole-1],existing=g[draft.target],any=g.center||g.front||g.back,view=draft.mapView?[draft.mapView.lat,draft.mapView.lng]:(existing||any||[34.1,-117.3]),zoom=draft.mapView?.zoom??(existing||any?18:10);map=L.map('courseMap').setView(view,zoom);
  ...
  Object.entries(g).forEach(([k,p])=>{if(p)L.circleMarker(p,{radius:8,color:k==='tee'?'#d8a93e':k==='front'?'#f4a340':k==='center'?'#176b45':'#174f9c',fillOpacity:.9}).addTo(map).bindTooltip(k)});map.on('click',e=>{draft.greens[draft.mapHole-1][draft.target]={lat:e.latlng.lat,lng:e.latlng.lng};render()});
}
...
function clearAllMarkers(){
  ...
  if(!markers.tee&&!markers.front&&!markers.center&&!markers.back){alert(`Hole ${hole} has no markers to clear.`);return}
  if(!confirm(`Clear the tee, front, center and back markers for Hole ${hole}?`))return;
  draft.greens[hole-1]={tee:null,front:null,center:null,back:null};render();
}
