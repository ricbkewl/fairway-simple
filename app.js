...
function updateShotPlanner(green){
  ...
  if(inlinePlannerLabels[0])inlinePlannerLabels[0].setLatLng(pointBetween(origin,aim,.5));
  const hit=$('plannerLineTargetYards'),left=$('plannerMarkerRemainingYards'),yardage=$('centerYards'),label=$('yardageTargetLabel');if(hit)hit.textContent=toTarget;if(left)left.textContent=remaining;if(yardage)yardage.textContent=routeTotal;if(label)label.textContent='Route Remaining';
...
function initInlineHoleMap(green){
  ...
  if(shotPlannerEnabled){
    shotPlannerGreen=green;const origin=shotPlannerOrigin(green),aim=shotPlannerAim(green),remainingPoints=remainingRoutePoints(origin,aim,green),icon=L.divIcon({className:'shot-planner-marker',html:'<span>◎<i><b id="plannerMarkerRemainingYards">—</b> yd to go</i></span>',iconSize:[42,42],iconAnchor:[21,21]}),lineLabel=L.divIcon({className:'planner-line-label-marker',html:'<span><b id="plannerLineTargetYards">—</b> yd<small>to hit</small></span>',iconSize:[1,1],iconAnchor:[0,0]});
    inlinePlannerLines=[L.polyline([origin,aim],{color:'#f5cf68',weight:5,opacity:1}).addTo(inlineHoleMap),L.polyline(remainingPoints,{color:'#fff',weight:3,opacity:.9,dashArray:'8 9'}).addTo(inlineHoleMap)];
    inlinePlannerMarker=L.marker(aim,{icon,interactive:true,keyboard:false,zIndexOffset:1200}).addTo(inlineHoleMap);inlinePlannerLabels=[L.marker(pointBetween(origin,aim,.5),{icon:lineLabel,interactive:false,keyboard:false,zIndexOffset:1100}).addTo(inlineHoleMap)];updateShotPlanner(green);
  }
