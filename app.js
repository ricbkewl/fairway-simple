...
async function loadGolferProfile(){
  ...
  if(!currentUser)return;
  const {data,error}=await db.from('golfer_profiles').select('first_name,last_name,phone').eq('user_id',currentUser.id).maybeSingle();
...
async function saveMyProfile(event){
  ...
  golferProfile={first_name:firstName,last_name:lastName,phone};alert('Your profile has been saved.');s.v='accountView';render();
}
