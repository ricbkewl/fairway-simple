/* ATG v184 — GPS-prioritized smart course discovery/search.
...
(function(){
  ...
  function quickChip(label,name,value=true){
    const on=name==='holes'?courseLibraryFilters.holes===Number(value):courseLibraryFilters[name]===true;
  ...
  function decorateCourses(){
    ...
    const sub=document.querySelector('h1 + .muted');if(sub)sub.textContent='Find nearby courses or search the complete ATG course library.';
    const resultSub=document.querySelector('.course-results-heading span');if(resultSub)resultSub.textContent='GPS-ready courses prioritized · search the full library';
  }
