/* Version 141: selectable Previous Matches with Select All and bulk delete-from-my-history. */
(function(){
  ...
  window.deleteSelectedHistoryMatches=async function(){
    ...
    if(!confirm(`Delete ${label} from your Previous Matches? This removes them only from your account history; other golfers keep their records.`))return;
  ...
  historyView=function(){
    ...
    app.innerHTML=`<button class="back" onclick="accountAction()">← Account</button>
      <div class="row"><div><h1>Previous Matches</h1><p class="muted">Every round played with this login is saved here.</p></div>${!historyLoading?'<button class="locate" onclick="openHistory()">Refresh</button>':''}</div>
