/* ATG v203 — scalable Filter & Sort Courses sheet. */
(function(){
  const STORE='atgCourseFilterSort:v203';
  ...
  function adminClass(c){return clean(c?.atgMappingClass||'').toLowerCase()}
  ...
  function chip(label,value,key='gpsStatus'){const on=extra[key]===value;return `<button type="button" class="atg150-chip ${on?'on':''}" data-extra-key="${key}" data-extra-value="${value}">${label}</button>`}
  function boolChip(label,key,legacy=false){const on=key==='holes18'?courseLibraryFilters.holes===18:key==='holes9'?courseLibraryFilters.holes===9:legacy?!!courseLibraryFilters[key]:!!extra[key];return `<button type="button" class="atg150-chip ${on?'on':''}" data-bool-key="${key}" data-legacy="${legacy?'1':'0'}">${label}</button>`}
  ...
  renderCourseFilterSheet=function(){
    ...
    sheet.classList.add('atg150-filter-sheet');
    sheet.innerHTML=`<header><button type="button" data-close>Cancel</button><b>Filter & Sort Courses</b><span class="atg150-count-badge">${filterCount()||''}</span></header><div class="course-filter-body atg150-body">
      <h3>GPS Status</h3><div class="atg150-grid">${chip('🟢 GPS Ready','ready')}${chip('🟡 Partial GPS','partial')}${chip('🟠 Course Located','located')}${chip('All','all')}</div>
      <h3>Location</h3><div class="atg150-grid compact">${boolChip('📍 Near Me','nearby',true)}</div><div class="atg150-selects"><select data-location="country">${optionList(countries,extra.country,'All Countries')}</select><select data-location="stateRegion">${optionList(states,extra.stateRegion,'All States / Provinces')}</select><select data-location="city">${optionList(cities,extra.city,'All Cities')}</select></div>
      <h3>Course</h3><div class="atg150-grid">${boolChip('18 Holes','holes18')}${boolChip('9 Holes','holes9')}${boolChip('Par 3','par3',true)}</div>
      <h3>My Courses</h3><div class="atg150-grid">${boolChip('★ Favorites','favorites',true)}${boolChip('↻ Recently Played','recent',true)}</div>
      <h3>Sort By</h3><div class="atg150-sort">${[['gps','GPS Ready First'],['nearest','Nearest'],['name','Course Name A–Z'],['recent','Recently Played']].map(([v,l])=>`<label><input type="radio" name="atg150-sort" value="${v}" ${extra.sortBy===v?'checked':''}><span>${l}</span></label>`).join('')}</div>
  ...
  function bindSheet(sheet){
    ...
    sheet.querySelectorAll('input[name="atg150-sort"]').forEach(r=>r.addEventListener('change',()=>{extra.sortBy=r.value;persist();refreshCourseLibrary();renderCourseFilterSheet();}));
  ...
  function decorateQuick(){
    ...
    row.querySelector('[data-indonesia-audit]')?.classList.add('atg150-hide-audit-chip');
  ...
  const style=document.createElement('style');style.textContent=`
    [data-indonesia-audit].atg150-hide-audit-chip{display:none!important}.atg150-filter-sheet{max-height:min(88vh,820px);overflow:hidden}.atg150-filter-sheet header{align-items:center}.atg150-count-badge{min-width:24px;height:24px;border-radius:12px;display:grid;place-items:center;background:#d4ad51;color:#14281f;font-weight:800;font-size:12px}.atg150-body{overflow:auto;padding-bottom:18px}.atg150-body h3{margin:18px 0 9px}.atg150-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.atg150-grid.compact{grid-template-columns:1fr}.atg150-chip,.atg150-indonesia{min-height:44px;border:1px solid rgba(126,113,72,.35);border-radius:12px;background:rgba(255,255,255,.72);font-weight:700}.atg150-chip.on{background:#153d2f;color:#fff;border-color:#d4ad51}.atg150-selects{display:grid;gap:8px;margin-top:8px}.atg150-selects select{width:100%;min-height:44px;border-radius:12px;padding:0 12px;border:1px solid rgba(126,113,72,.3);background:#fff}.atg150-sort{display:grid;gap:7px}.atg150-sort label{display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid rgba(126,113,72,.25);border-radius:12px;background:rgba(255,255,255,.65)}.atg150-admin{margin-top:18px;padding:12px;border-radius:15px;background:rgba(19,61,47,.08);border:1px solid rgba(19,61,47,.18)}.atg150-indonesia{width:100%;margin-top:8px;background:#173d30;color:#fff;border-color:#d4ad51}.course-filter-sheet footer .primary{background:#173d30;color:#fff;border-color:#d4ad51}.course-discovery-tools [onclick*="showCourseFilters"]{position:relative}.course-discovery-tools [onclick*="showCourseFilters"][data-active-filters]:not([data-active-filters="0"])::after{content:attr(data-active-filters);position:absolute;top:-6px;right:-6px;min-width:18px;height:18px;padding:0 4px;border-radius:9px;background:#d4ad51;color:#14281f;font:800 11px/18px system-ui;text-align:center}
