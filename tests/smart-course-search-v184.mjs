...
vm.runInContext(fs.readFileSync(path.join(root,'gps-search-priority-v186.js'),'utf8'),context);
context.courseLibraryQuery='spring';
assert.equal(context.rankedSharedCourses()[0].course.id,'ready');
context.courseLibraryQuery='spring valley';
assert.deepEqual(Array.from(context.rankedSharedCourses().filter(r=>r.searchMatch),r=>r.course.id),['missing']);
context.courseLibraryFilters={holes:null};
context.document.getElementById=()=>null;
context.smartQuickFilter('holes','18');
assert.equal(context.courseLibraryFilters.holes,18);
context.smartQuickFilter('holes','18');
assert.equal(context.courseLibraryFilters.holes,null);
console.log('Combined GPS-first stack and numeric quick filters passed.');
