/* Version 149: populate ATG search from the shared Golf Course Library.
...
(function(){
  ...
  function validPoint(p){return p&&p.lat!==null&&p.lng!==null&&p.lat!==''&&p.lng!==''&&Number.isFinite(Number(p.lat))&&Number.isFinite(Number(p.lng))&&Math.abs(Number(p.lat))<=90&&Math.abs(Number(p.lng))<=180&&!(Number(p.lat)===0&&Number(p.lng)===0)}
  ...
  function mergeCatalog(rows){
    ...
    for(let i=0;i<courses.length;i++){
      const c=courses[i];const key=normalize(c.name);byName.set(key,[...(byName.get(key)||[]),i]);if(c.sharedCourseId)byShared.set(String(c.sharedCourseId),i);
    }
    ...
    for(const row of rows){
      ...
      if(idx===undefined){
        const candidates=(byName.get(normalize(row.name))||[]).filter(i=>{
          const c=courses[i];
          if(c.sharedCourseId&&String(c.sharedCourseId)!==String(row.shared_course_id))return false;
          return [['city',row.city],['state',row.state_code||row.state],['country_code',row.country_code]].every(([key,value])=>!c[key]||!value||normalize(c[key])===normalize(value));
        });
        if(candidates.length===1)idx=candidates[0];
      }
      const catalogPoint=point(row);
      if(idx!==undefined){
        const prior=courses[idx];
        courses[idx]={...prior,
          city:row.city||prior.city,
          address:row.address||prior.address,postal_code:row.postal_code||prior.postal_code,
      ...
      courses.push(item);byName.set(normalize(item.name),[...(byName.get(normalize(item.name))||[]),courses.length-1]);byShared.set(String(row.shared_course_id),courses.length-1);added++;
    }
  ...
  async function hydrateSafeSharedCourses(rows){
    ...
    for(let i=0;i<wanted.length;i+=6){
      ...
      for(const result of results){
        ...
        const idx=courses.findIndex(c=>String(c.sharedCourseId||'')===String(result.row.shared_course_id));
        if(idx<0)continue;
        ...
        const pars=incomingPars.length===holeCount&&incomingPars.every(p=>Number.isFinite(p)&&p>=2&&p<=7)?incomingPars:null;
        const greens=safeGps(payload,prior);
        // Shared reference updates must not replace ATG editors' saved geometry.
        if(!String(prior.id).startsWith('shared-')&&(prior.greens||[]).some(g=>validPoint(g?.tee)||validPoint(g?.center))){hydrated++;continue;}
  ...
  async function loadSharedCourseLibrary({rerender=true,retries=6}={}){
    ...
    sharedLoadPromise=(async()=>{
      const stats={version:149,loaded:false,catalogRows:0,added:0,matched:0,hydrated:0,gpsActive:0,error:null,loadedAt:null};
      try{
        ...
        if(rerender&&typeof refreshCourseLibrary==='function')refreshCourseLibrary();
  ...
  if(typeof loadCourses==='function'){
    ...
    loadCourses=async function(){
      // Finish an in-flight merge before the private ATG refresh replaces courses.
      if(sharedLoadBusy)await sharedLoadPromise;
      const retained=(courses||[]).filter(c=>c.sharedCourseId);
      await priorLoadCourses145();
      const ids=new Set(courses.map(c=>c.id));
      for(const c of retained)if(!ids.has(c.id))courses.push(c);
